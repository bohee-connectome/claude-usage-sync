...
""")
    ...
    config = {
        ...
        'setup_date': datetime.now(KST).isoformat()
    }
...
def sync_usage():
    ...
    try:
        ...
        commit_msg = f"Update usage from {device_id} - {datetime.now(KST).strftime('%Y-%m-%d %H:%M KST')}"
        ...
        if result.returncode == 0:
            ...
            if push_result.returncode == 0:
                ...
                print("⚠️  Push failed (may need to set up remote)")
                print(push_result.stderr)
        else:
            if "nothing to commit" in result.stdout:
                print("ℹ️  No changes since last sync")
            else:
    ...
    print("✅ Sync complete!")
    print()
    print("💡 To see combined usage from all devices:")
    print("   ccusage-total")
    print("=" * 70)
